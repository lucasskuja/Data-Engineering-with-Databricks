...
class DBAcademyHelper:
    def __init__(self, lesson=None):
        ...
        self.working_dir_prefix = (
            f"dbfs:/user/{self.username}/dbacademy/{self.course_name}"
        )
    ...
    def init(self, create_db=True):
        ...
        if create_db:
            ...
            spark.sql(
                f"CREATE DATABASE IF NOT EXISTS {self.db_name} LOCATION '{self.paths.user_db}'"
            )
    ...
    def conclude_setup(self):
        ...
        if self.source_db_name:
            ...
            tables = (
                spark.sql(f"SHOW TABLES IN {self.source_db_name}")
        ...
        if self.create_db:
            ...
            tables = (
                spark.sql(f"SHOW TABLES IN {self.db_name}")
                .filter("isTemporary == false")
                .select("tableName")
                .collect()
            )
...
def install_dtavod_datasets(reinstall):
    ...
    copy_source_dataset(
        f"{DA.paths.datasets}/flights/departuredelays.csv",
...
def load_eltwss_external_tables():
    copy_source_dataset(
        f"{DA.paths.datasets}/raw/sales-csv",
        f"{DA.paths.working_dir}/sales-csv",
        "csv",
        "sales-csv",
    )
    ...
    dbutils.fs.cp(
        f"{DA.paths.datasets}/raw/users-historical",
        f"{DA.paths.working_dir}/users-historical",
        True,
    )
...
def create_eltwss_users_update():
    ...
    # REFACTORING - Making lesson-specific copy
    dbutils.fs.cp(
        f"{DA.paths.datasets}/raw/users-30m", f"{DA.paths.working_dir}/users-30m", True
    )
    ...
    spark.sql(
        "INSERT INTO users_dirty VALUES (NULL, NULL, NULL, NULL), (NULL, NULL, NULL, NULL), (NULL, NULL, NULL, NULL)"
    )
