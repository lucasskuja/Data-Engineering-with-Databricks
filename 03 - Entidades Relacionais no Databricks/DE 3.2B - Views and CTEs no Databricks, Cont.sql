-- Databricks notebook source
-- MAGIC %md
-- MAGIC
-- MAGIC # Views and CTEs on Databricks, Continuação
-- MAGIC
-- MAGIC Estamos continuando do notebook [DE 3.2A - Views and CTEs on Databricks]($./DE 3.2A - Views and CTEs on Databricks) onde acabamos de revisar as seguintes duas listas de tabelas<br/>
-- MAGIC e views, com a observação especial de que nossa view temporária global **`global_temp_view_dist_gt_1000`** não estava incluída na primeira lista.
-- MAGIC

-- COMMAND ----------

-- MAGIC %md-sandbox
-- MAGIC
-- MAGIC
-- MAGIC
-- MAGIC <img src="data:image/png;base64," />

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC Com o estado do notebook resetado, precisamos re-inicializar algumas das configurações específicas da nossa lição.
-- MAGIC
-- MAGIC Nota: **NÃO** iremos recriar o banco de dados para a segunda metade desta lição.
-- MAGIC

-- COMMAND ----------

-- MAGIC %run ../Includes/Classroom-Setup-3.2B

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC Mas, precisamos configurar esta sessão para usar nosso banco de dados como padrão.
-- MAGIC

-- COMMAND ----------

USE ${da.db_name};

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC Finalmente, execute as duas células abaixo para confirmar que:
-- MAGIC 1. A tabela **`external_table`** ainda existe.
-- MAGIC 2. A view **`view_delays_abq_lax`** ainda existe.
-- MAGIC 3. A view temporária **`temp_view_delays_gt_120`** **NÃO** existe.
-- MAGIC 4. A view temporária global **`global_temp_view_dist_gt_1000`** existe no banco de dados especial **`global_temp`**.
-- MAGIC
-- MAGIC <img src="https://files.training.databricks.com/images/icon_hint_24.png"> Dica: Se você voltasse ao notebook anterior e executasse novamente **`SHOW TABLES`**<br/>
-- MAGIC todas as três tabelas e views do banco de dados atual ainda seriam mostradas.
-- MAGIC

-- COMMAND ----------

SHOW TABLES;

-- COMMAND ----------

SHOW TABLES IN global_temp;

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC Como mencionado anteriormente, views temporárias estão ligadas a uma sessão Spark e, portanto, não estão acessíveis...
-- MAGIC * Após reiniciar um cluster
-- MAGIC * Após desanexar e reanexar a um cluster
-- MAGIC * Após instalar um pacote Python que reinicia o interpretador Python
-- MAGIC * Ou de outro notebook
-- MAGIC
-- MAGIC ...com a exceção especial das views temporárias globais.
-- MAGIC

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC Views temporárias globais funcionam de maneira muito similar às outras views temporárias, mas diferem em um ponto importante.
-- MAGIC
-- MAGIC Elas são adicionadas ao banco de dados **`global_temp`** que existe no **`cluster`**.
-- MAGIC
-- MAGIC Enquanto o cluster estiver rodando, esse banco de dados persiste e quaisquer notebooks conectados ao cluster podem acessar suas views temporárias globais.  
-- MAGIC   
-- MAGIC Podemos ver isso em ação executando as células a seguir:
-- MAGIC

-- COMMAND ----------

SELECT * FROM global_temp.global_temp_view_dist_gt_1000;

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC Views temporárias globais são "perdidas" quando o cluster é reiniciado.
-- MAGIC
-- MAGIC Confie na gente, não faça isso agora, mas se você reiniciar o cluster, a instrução select acima falharia porque a tabela não existiria mais.
-- MAGIC

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC ## Expressões de Tabela Comuns (CTEs)
-- MAGIC
-- MAGIC As CTEs podem ser usadas em uma variedade de contextos. Abaixo estão alguns exemplos das diferentes formas de utilizar uma CTE em uma consulta. Primeiro, um exemplo de como criar múltimos aliases de colunas usando uma CTE.
-- MAGIC

-- COMMAND ----------

WITH flight_delays(
  total_delay_time,
  origin_airport,
  destination_airport
) AS (
  SELECT
    delay,
    origin,
    destination
  FROM
    external_table
)
SELECT
  *
FROM
  flight_delays
WHERE
  total_delay_time > 120
  AND origin_airport = "ATL"
  AND destination_airport = "DEN";

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC A seguir, um exemplo de uma CTE dentro da definição de outra CTE.
-- MAGIC

-- COMMAND ----------

WITH lax_bos AS (
  WITH origin_destination (origin_airport, destination_airport) AS (
    SELECT
      origin,
      destination
    FROM
      external_table
  )
  SELECT
    *
  FROM
    origin_destination
  WHERE
    origin_airport = 'LAX'
    AND destination_airport = 'BOS'
)
SELECT
  count(origin_airport) AS `Total Flights from LAX to BOS`
FROM
  lax_bos;

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC Agora, aqui está um exemplo de uma CTE dentro de uma subconsulta.
-- MAGIC

-- COMMAND ----------

SELECT
  max(total_delay) AS `Longest Delay (in minutes)`
FROM
  (
    WITH delayed_flights(total_delay) AS (
      SELECT
        delay
      FROM
        external_table
    )
    SELECT
      *
    FROM
      delayed_flights
  );

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC Também podemos usar uma CTE dentro de uma expressão de subconsulta.
-- MAGIC

-- COMMAND ----------

SELECT
  (
    WITH distinct_origins AS (
      SELECT DISTINCT origin FROM external_table
    )
    SELECT
      count(origin) AS `Number of Distinct Origins`
    FROM
      distinct_origins
  ) AS `Number of Different Origin Airports`;

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC Por fim, aqui está uma CTE em uma instrução **`CREATE VIEW`**.

-- COMMAND ----------

CREATE OR REPLACE VIEW BOS_LAX 
AS WITH origin_destination(origin_airport, destination_airport) 
AS (SELECT origin, destination FROM external_table)
SELECT * FROM origin_destination
WHERE origin_airport = 'BOS' AND destination_airport = 'LAX';

SELECT count(origin_airport) AS `Number of Delayed Flights from BOS to LAX` FROM BOS_LAX;

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC ## Limpeza  
-- MAGIC Primeiro, vamos remover o banco de dados de treinamento.
-- MAGIC

-- COMMAND ----------

DROP DATABASE ${da.db_name} CASCADE;

-- COMMAND ----------

-- MAGIC %md
-- MAGIC
-- MAGIC Execute a célula abaixo para deletar as tabelas e arquivos associados a esta lição.
-- MAGIC

-- COMMAND ----------

-- MAGIC %python 
-- MAGIC DA.cleanup()

-- COMMAND ----------

-- MAGIC %md-sandbox
-- MAGIC &copy; 2022 Databricks, Inc. Todos os direitos reservados.<br/>
-- MAGIC Apache, Apache Spark, Spark e o logotipo do Spark são marcas registradas da <a href="https://www.apache.org/">Apache Software Foundation</a>.<br/>
-- MAGIC <br/>
-- MAGIC <a href="https://databricks.com/privacy-policy">Política de Privacidade</a> | <a href="https://databricks.com/terms-of-use">Termos de Uso</a> | <a href="https://help.databricks.com/">Suporte</a>
-- MAGIC
